...
class MinimumEnergyDisambiguator:
    ...
    def __init__(self, Bx, By, Bz, az=None, dx=1.0, dy=1.0, lambda_factor=1.0, pad_width=3, polar=False, gamma=np.deg2rad(65)):
        ...
        if az is not None:
            ...
            self.az = (np.degrees(self.az)) % 360
            self.az = np.deg2rad(self.az)
        self.dx = dx
        self.dy = dy
    ...
    def compute_dBz_dz_custom(self, Bz_arr):
       
        pad = self.pad_width
        Bz_padded = np.pad(Bz_arr, pad_width=pad, mode='constant', constant_values=0)
        N0, N1 = Bz_padded.shape
        taper0 = self.cos_taper(N0, pad)
        taper1 = self.cos_taper(N1, pad)
        window = np.outer(taper0, taper1)
        ...
        dBz_dz_fft = -np.sqrt(K2) * Bz_fft
        dBz_dz_padded = np.real(np.fft.ifft2(dBz_dz_fft))
        dBz_dz = dBz_dz_padded[pad:pad + Bz_arr.shape[0], pad:pad + Bz_arr.shape[1]]
        return dBz_dz
    ...
    def compute_derivatives(self):
        ...
        if self.polar:
            ...
            Bx_h = self.Bx * np.cos(gamma) - self.Bz * np.sin(gamma)
            By_h = self.By  # остаје исто
            Bz_h = self.Bx * np.sin(gamma) + self.Bz * np.cos(gamma)
           
            self.Bx_h = Bx_h
            self.By_h = By_h
            self.Bz_h = Bz_h
            
            az_temp = np.arctan2(By_h, Bx_h)
            az_temp = (np.degrees(az_temp)) % 360
            self.az = np.deg2rad(az_temp)
        
            Bx_ap = self.apodize_array(Bx_h)
            By_ap = self.apodize_array(By_h)
            Bz_ap = self.apodize_array(Bz_h)
           
        else:
            Bx_ap = self.apodize_array(self.Bx)
            By_ap = self.apodize_array(self.By)
            Bz_ap = self.apodize_array(self.Bz)
        ...
        if self.polar:
            dBz_dz = self.compute_dBz_dz_custom(Bz_ap)
        else:
    ...
    def simulated_annealing(self, neq=100, tfac0=0.1, tfactr=0.9,
        ...
        E_prev = E
        nconv = 0
        for _ in tqdm(range(max_iter), desc='Disambiguation [strong-masked fields] - simulated_annealing'):
            nsucc = 0
            for _ in range(neq):
    ...
    def acute_angle_weak(self, Btr_threshold=100.0, max_iterations=10, conv_threshold=1e-2):
        ...
        for it in range(max_iterations):
            ...
            if change < conv_threshold:
                print(f'Acute-angle method [weak fields] converged after {it + 1} iterations')
                break
